...
export async function createInvoiceWithMonitoring(invoiceData, userSession) {
    ...
    await monitor.trackInvoiceLifecycle(invoiceData.invoice_id.toString(), {
        ...
        onFunded: async (data) => {
            ...
            window.dispatchEvent(new CustomEvent('invoice-funded', {
                detail: {
                    invoiceId: invoiceData.invoice_id,
                    txId: data.tx_id,
                    timestamp: new Date(),
        ...
        onReleased: async (data) => {
            ...
            window.dispatchEvent(new CustomEvent('invoice-released', {
                detail: {
                    invoiceId: invoiceData.invoice_id,
                    txId: data.tx_id,
                    timestamp: new Date(),
        ...
        onDisputed: async (data) => {
            ...
            window.dispatchEvent(new CustomEvent('invoice-disputed', {
                detail: {
                    invoiceId: invoiceData.invoice_id,
                    txId: data.tx_id,
                    timestamp: new Date(),
...
export async function getInvoiceRealtimeStatus(invoiceId) {
    const contractAddress = import.meta.env.VITE_CONTRACT_ADDRESS || 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM';
    const monitor = new InvoiceBlockchainMonitor(hiroAPI, contractAddress);
    try {
