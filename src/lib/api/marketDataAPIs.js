...
export class CoinMarketCapAPI {
    ...
    async getCryptoPrices(symbols = ['BTC', 'STX']) {
        try {
    ...
    async getMarketMetrics(symbol = 'BTC') {
        try {
...
export class AlphaVantageAPI {
    ...
    async getExchangeRates() {
        try {
            const response = await axios.get(`https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=USD&to_currency=BTC&apikey=${this.apiKey}`);
            return response.data;
        }
        catch (error) {
    ...
    async getForexRates(fromSymbol = 'USD', toSymbol = 'EUR') {
        try {
            const response = await axios.get(`https://www.alphavantage.co/query?function=FX_DAILY&from_symbol=${fromSymbol}&to_symbol=${toSymbol}&apikey=${this.apiKey}`);
            return response.data;
        }
        catch (error) {
            console.error('AlphaVantage API error:', error);
